...
MAX_ITERATIONS = 8  # guardrail against infinite loops
...
a coding task involving a small Python project. You have tools to read files, write \
files, run the test suite, and list files.

Your process should be:
1. Run the tests first to see what's failing (if anything).
2. Read the relevant source file(s) to understand the code.
3. Reason carefully about the ACTUAL bug - do not guess blindly.
4. Apply a fix using write_file (always write the complete file content).
5. Run the tests again to confirm the fix works.
6. If tests still fail, read the new output carefully and try again.
...
if __name__ == "__main__":
    task = (
        "There is a bug somewhere in calculator.py causing a test to fail. "
        "Find it, fix it, and confirm all tests pass."
    )
    run_agent(task)
